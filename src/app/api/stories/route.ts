...
export async function GET(req: NextRequest) {
  ...
  if (FIREBASE_SUPPORTED.includes(type)) {
    try {
      ...
      console.error(`Firebase API failed for ${type}, falling back to Algolia:`, err);
...
function mapAlgoliaHits(hits: any[]) {
  return hits
    ...
    .map((hit) => ({
      ...
      type: hit._tags?.includes('ask_hn') ? 'ask' :
            hit._tags?.includes('show_hn') ? 'show' :
            hit._tags?.includes('job') ? 'job' : 'story',
    }));
}
